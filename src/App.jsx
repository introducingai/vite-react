    ...
    <div style={{ position:"relative",overflow:"hidden",background:"linear-gradient(135deg,#0c1525 0%,#070b14 55%,#0a0508 100%)",border:"1px solid rgba(230,48,48,0.18)" }}>
      ...
      <div style={{ position:"relative",zIndex:1,display:"grid",gridTemplateColumns:"1fr 120px" }}>
        ...
        <div style={{ padding:"28px 28px 24px" }}>
          ...
          <div style={{ marginTop:20,paddingTop:16,borderTop:"1px solid rgba(255,255,255,0.05)",display:"flex",gap:10,flexWrap:"wrap",alignItems:"center" }}>
            {entry.source_url ? (
              <a href={entry.source_url} target="_blank" rel="noopener noreferrer"
                style={{ display:"inline-flex",alignItems:"center",gap:7,fontFamily:"monospace",fontSize:10,color:"#e63030",textDecoration:"none",letterSpacing:"0.12em",textTransform:"uppercase",border:"1px solid rgba(230,48,48,0.3)",padding:"8px 16px",background:"rgba(230,48,48,0.07)",transition:"background 0.15s" }}
                onMouseEnter={e=>e.currentTarget.style.background="rgba(230,48,48,0.15)"}
                onMouseLeave={e=>e.currentTarget.style.background="rgba(230,48,48,0.07)"}
              >
                {entry.source_url.includes("x.com") || entry.source_url.includes("twitter.com") ? "↗ View on X" : entry.source_url.includes("github.com") ? "↗ View on GitHub" : "↗ View source"}
              </a>
            ) : (
              <span style={{ fontFamily:"monospace",fontSize:9,color:"rgba(255,255,255,0.12)",letterSpacing:"0.1em" }}>no source url — add one when profiling</span>
            )}
            {entry.product_url && entry.product_url !== entry.source_url && (
              <a href={entry.product_url} target="_blank" rel="noopener noreferrer"
                style={{ display:"inline-flex",alignItems:"center",gap:7,fontFamily:"monospace",fontSize:10,color:"rgba(255,255,255,0.5)",textDecoration:"none",letterSpacing:"0.12em",textTransform:"uppercase",border:"1px solid rgba(255,255,255,0.1)",padding:"8px 16px",background:"rgba(255,255,255,0.03)",transition:"background 0.15s" }}
